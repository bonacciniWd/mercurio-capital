-- Recuperacao manual: restaura a RPC anterior. Nao apaga dados nem altera historico de migrations.
BEGIN;
-- Volume liberado follows the effective credit date, not proposal creation date.
...
begin
  ...
  select jsonb_build_object(
    ...
    'volume_liberado',coalesce((select sum(valor_liberado) from liberacoes where status in ('pagamento_comissao','completo')),0),
    ...
    'top_parceiros',coalesce((select jsonb_agg(to_jsonb(t)) from (
      select p.partner_id,u.nome_completo partner_nome,count(*)::int total,
        ...
        coalesce(sum(l.valor_liberado) filter(where l.status in ('pagamento_comissao','completo')),0) volume_liberado
      from periodo p join public.partners pa on pa.id=p.partner_id join public.usuarios u on u.id=pa.usuario_id
      left join liberacoes l on l.proposta_id=p.id
