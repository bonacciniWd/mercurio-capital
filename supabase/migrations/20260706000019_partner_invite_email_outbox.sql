-- =============================================
-- FEATURE: Convite de equipe com e-mail transacional automático
-- Data: 2026-07-06
-- =============================================

create extension if not exists pgcrypto with schema extensions;

-- Template padrão para convite de equipe (canal e-mail).
insert into public.templates_mensagem (codigo, canal, nome, assunto, corpo, variaveis, ativo)
values (
  'convite_equipe_v1',
  'email',
  'Convite para equipe do parceiro',
  ...
  '{nome,email,parceiro_exibicao,equipe_nome,convite_link,expires_in_min}',
  true
)
...
declare
  ...
  v_base_url              text := 'https://app.mercuriocapital.com.br';
  v_convite_link          text;
...
begin
  ...
  begin
    ...
    where c.chave in ('app_url', 'frontend_url', 'site_url')
    order by case c.chave when 'app_url' then 1 when 'frontend_url' then 2 else 3 end
    limit 1;
  exception when others then
    v_base_url := 'https://app.mercuriocapital.com.br';
  end;

  v_base_url := coalesce(
    nullif(regexp_replace(coalesce(v_base_url, ''), '/+$', ''), ''),
    'https://app.mercuriocapital.com.br'
  );
  ...
  -- Template dinâmico com fallback textual para não bloquear o convite.
  begin
