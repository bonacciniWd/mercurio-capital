-- =============================================
-- MIGRATION — partner_create_proposta: persistir detalhes do imóvel
-- (valor_aluguel, instituicao_financiadora, saldo_devedor,
--  debitos_iptu, debitos_condominio)
--
-- As colunas já existem em `imoveis` desde 20260513000003_originacao.sql,
-- mas a RPC original (20260514000003) não as inseria. Esta migration
-- recria a função com o INSERT estendido.
-- =============================================
...
begin
  ...
  -- Imóveis (agora com detalhes condicionais aos toggles)
  for v_imovel in select * from jsonb_array_elements(p_payload->'imoveis') loop
    insert into imoveis (
      ...
      case when coalesce((v_imovel->>'alugado')::boolean, false)
           then nullif(v_imovel->>'valor_aluguel','0')::numeric
           else null end,
      coalesce((v_imovel->>'financiado')::boolean, false),
      case when coalesce((v_imovel->>'financiado')::boolean, false)
           then nullif(v_imovel->>'instituicao_financiadora','')
           else null end,
      case when coalesce((v_imovel->>'financiado')::boolean, false)
           then nullif(v_imovel->>'saldo_devedor','0')::numeric
           else null end,
      coalesce((v_imovel->>'possui_debitos')::boolean, false),
      case when coalesce((v_imovel->>'possui_debitos')::boolean, false)
           then nullif(v_imovel->>'debitos_iptu','0')::numeric
           else null end,
      case when coalesce((v_imovel->>'possui_debitos')::boolean, false)
           then nullif(v_imovel->>'debitos_condominio','0')::numeric
           else null end
