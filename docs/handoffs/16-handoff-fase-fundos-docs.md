# Handoff — Fase Fundos & Documentos (Admin limitado, Fundos, Checklist, Modelo de contrato)
...
**Modelo de contrato:**
- `proposta_contrato_modelo_add(p_proposta_id, p_storage_path, p_nome_arquivo)` — guard `app_is_admin()`, valida path `{proposta_id}/…`.
- `proposta_contrato_modelo_remove(p_id)` — admin; retorna `storage_path` para limpeza do bucket.

---

## 4. RLS

- `fundos` e `proposta_fundos`: **admin-only** (`for all using app_is_admin()`), sem policy para partner/client ⇒ sem acesso.
...
- `proposta_contrato_modelos`: `all` admin; `select` para parceiro dono (`partner_id = app_partner_id()` ou `equipe_id = app_equipe_id()`) e cliente da proposta.
...
2. Rodar os 4 smoke tests contra `bhagksfvszeogtjvjtpx`.
3. Promover um admin de teste: `select public.admin_set_admin_nivel('<uuid>', 'limitado');` (exige admin full) e refazer login para o JWT trazer o claim.
